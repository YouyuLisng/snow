...
public partial class control_contactform3 : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            //if()
            //FormContainer.Style.Add("background-color", BackgroupColor.ToString());
            //FormContainer.Style.Add("background-color", "rgb(" + BackgroupColor.R + "," + BackgroupColor.G + "," + BackgroupColor.B + ")");
            //contact.Style.Add("background-color", "rgb(" + backgroundcolor.R +"," + backgroundcolor.G+","+ backgroundcolor.B+")");
            Form.Style.Add("background-color", "rgb(" + backgroundcolor.R +"," + backgroundcolor.G+","+ backgroundcolor.B+")");
        }
        //else {
        //    //FormContainer.Style.Add("background-color", "rgb(" + BackgroupColor.R + "," + BackgroupColor.G + "," + BackgroupColor.B + ")");
        //    //contact.Style.Add("background-color", "#FFFFFF");
        //    contact.Style.Add("background-color", "rgb(255,255,255)");

        //}
        if (TitleImgUrl != null)
        {
            ContactformTitleImg.ImageUrl = TitleImgUrl;
        }
        else {
            Lierror.Text = "當你看到這段文字的時候 代表你沒有指定標題圖片的路徑 請在contactform3地控制項裡 設定 TitleImgUrl=\"圖片路徑\"";
        }
            ...
            if (ProductID > 0 || ProductSchID > 0)
            {
                using (DataModel dm = new DataModel())
                {
                    ...
                    if (product != null)
                    {
                        ...
                        ProductNm = "行程名稱:" + product.ProductNm;
                        //ProductSD = 
                        sProductID = "行程編號:" + ProductID;
                        LabProductNm.Text = product.ProductNm;
                        PHProduct.Visible = true;
                    }

                    var productsch = dm.tblProductSch.Include("tblProduct").Where(a => a.ProductSchID == ProductSchID).FirstOrDefault();
                    if (productsch != null)
                    {
                        ProductNm = "行程名稱:" + product.ProductNm;
                        ProductSchSD = "出發日期:" + productsch.ProductSchSD.ToString("yyyy/MM/dd");
                        sProductID = "行程編號:" + ProductID;
                        //sProductSchID = "行程編號:"+ ProductSchID;
                        sProductTourID = "團號:" + productsch.ProductTourID;
                        LabProductNm.Text = product.ProductNm;
                        LabProductTourID.Text = productsch.ProductTourID.ToString();
                        LabProductSchSD.Text = productsch.ProductSchSD.ToString("yyyy/MM/dd");
                        PHProduct.Visible = true;
    ...
    protected void BtnPostMsg_Click(object sender, EventArgs e)
    {
        if (MailTitle == null) {
            ...
            MailTitle = "此活動尚未設定諮詢單名稱!!!";
        }
        //團名
        string TourName = HttpContext.Current.Request.QueryString["ProductNm"] ?? "";
        //團號
        string TourID = HttpContext.Current.Request.QueryString["ProductTourID"] ?? "";
        ...
        if (IsCaptchaValid)
        {
            ...
            if (vCheck == "1")
            {
                ...
                //    {
                //        var mailgroupdts = dm.tblMailGroupDTS.Where(a => a.MailGroupName == mailgroup);

                //        //var login = dm.tblLogin.Where(a => a.LoginEnable && !a.LoginDel && a.LoginEmail.Length > 0 && a.tblMailGroupDTS.Any(b => b.MailGroupName == mailgroup));
                //        var login = dm.tblMailGroupItemDTS.Where(a => a.tblLogin.LoginEnable && !a.tblLogin.LoginDel && a.tblLogin.LoginEmail.Length > 0 && a.tblMailGroupDTS.MailGroupName == mailgroup);
                ...
                //        var loginD = bpdm.tblLoginD.Where(a => a.LoginEnable && !a.LoginDel && a.LoginEmail.Length > 0 && a.tblMailGroup.Any(b => b.MailGroupName == mailgroup));
